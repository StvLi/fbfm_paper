...
| 11 | Braun et al., *Riemannian Flow Matching Policy*, 2024 | 将 Flow Matching policy 定义在机器人状态的 Riemannian manifold 上，生成平滑多模态轨迹。 | 证明机器人 Flow-Matching policy 早于 \(\pi_0\)。 | Core |
...
3. **robot Flow Matching → \(\pi\) series**：RFMP/multi-support/affordance-conditioned flows 证明 \(\pi_0\) 不是首个机器人 Flow-Matching policy，再独立说明 \(\pi_0\) 和 \(\pi_{0.5}\) 的整合性贡献。
...
Flow Matching policies emerged before and alongside large VLA action experts. RFMP transported robot motions on Riemannian state spaces, while related work applied Flow Matching to multi-support whole-body imitation and affordance-conditioned manipulation [Braun et al., 2024; Rouxel et al., 2024; Zhang and Gienger, 2025]. These studies make clear that \(\pi_0\)'s contribution is not the first use of Flow Matching for robot motion. Its distinctive step is integrative: \(\pi_0\) pairs a pretrained VLM backbone with a smaller robotics-specific action expert trained by conditional Flow Matching, and combines this architecture with cross-embodiment pretraining and post-training to generate continuous high-frequency action chunks for dexterous control [Black et al., 2024]. \(\pi_{0.5}\) retains this policy family while adding heterogeneous co-training and semantic subtask prediction for open-world, long-horizon manipulation [Physical Intelligence et al., 2025]. This line provides the action-generation side of FBFM; the next theme considers how such iteratively generated chunks can remain responsive during execution.
...
Flow-Matching policy 的出现早于、也伴随着大规模 VLA action expert 的发展。RFMP 在 Riemannian state space 上输运机器人运动，相关工作还把 Flow Matching 用于 multi-support whole-body imitation 和 affordance-conditioned manipulation [Braun et al., 2024; Rouxel et al., 2024; Zhang and Gienger, 2025]。因此，\(\pi_0\) 的贡献不能表述为首次将 Flow Matching 用于机器人运动。它的独特增量是整合性的：\(\pi_0\) 将预训练 VLM backbone 与较小的机器人专用 action expert 结合，使用 conditional Flow Matching 训练后者，并配合 cross-embodiment pretraining 与 post-training，为灵巧控制生成连续高频 action chunks [Black et al., 2024]。\(\pi_{0.5}\) 保留这一策略家族，同时加入异构数据 co-training 与 semantic subtask prediction，以支持开放世界长时域操作 [Physical Intelligence et al., 2025]。这条路线构成 FBFM 的动作生成侧背景；下一主题将讨论这类迭代生成的 action chunk 如何在执行过程中保持响应性。
