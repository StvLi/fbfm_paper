...
3. **specialized robot Flow Matching → \(\pi\) series**：RFMP/multi-support/affordance-conditioned flows 补全专用机器人运动策略的发展背景；随后重点说明 \(\pi_0\) 和 \(\pi_{0.5}\) 如何将这条路线扩展为受到广泛认可的大规模通用 VLA，并以应用性能体现其现实价值。
