...
% This file mirrors Preliminaries_and_Notations.md. Keep both versions aligned.
...
\begin{table*}[t]
    ...
    \begin{tabular}{p{0.23\textwidth} p{0.71\textwidth}}
        ...
        $H,d_a,D$
        & Prediction horizon, dimension of one action, and flattened dimension of the generated chunk or mask.\\
        $a_t,\mathbf{A}_t$
        & A single-step action and the action chunk $[a_t,\ldots,a_{t+H-1}]$.\\
        $z_t,\mathbf{Z}_t,\mathcal{Z}$
        & An encoded single-step latent state, the future latent-state chunk $[z_{t+1},\ldots,z_{t+H}]$, and the latent space.\\
        $x,\mathbf{X}_t$
        ...
        & Gaussian source noise, zero vector, identity matrix, and the intermediate chunk on the probability path.\\
        ...
        $\mathbf{W}_t,\odot$
        & Element-wise feedback mask or confidence weights and the Hadamard product.\\
        ...
        $\operatorname{vec},\operatorname{concat}$
        & Flattening and concatenation operators used to construct a joint chunk.\\
        \hline
    \end{tabular}
...
\begin{table*}[t]
    ...
    \begin{tabular}{p{0.29\textwidth} p{0.65\textwidth}}
        ...
        $d_z,D_Z,D_A,D_X$
        & Dimension of one flattened latent-state slot; flattened state, action, and joint chunk dimensions, with $D_Z=Hd_z$ and $D_X=D_Z+D_A$.\\
