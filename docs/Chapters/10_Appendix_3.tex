...
implementation is on branch \texttt{fix/dreamzero-rolling-feedback} at commit
\texttt{5300093}; the standalone A6000 deployment used by the ongoing LIBERO
experiment is on branch \texttt{runnable-dreamzero-fbfm-libero} at commit
\texttt{a7dcd4a}. The latter is the reference implementation for the DreamZero
results. The default \texttt{main} branch does not contain the causal rolling
FBFM path and must not be used to reproduce this track.
