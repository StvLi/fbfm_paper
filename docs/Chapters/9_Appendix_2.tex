...
\section{Pseudoinverse Interpretation and Approximation Details}
\label{app:pseudoinverse}

\subsection{Generalized-Inverse Consistency}
...
For a nonlinear measurement operator $h$, Song et al.~\cite{song2023pseudoinverse}
require a generalized inverse satisfying
\begin{equation}
...
\end{equation}
These identities motivate the term \emph{Moore--Penrose-consistent feedback
...
\subsection{Masked Inpainting as a Projection}
...
\subsection{Aligned-Coordinate Approximation}
...
\subsection{Few-Step Flow-Matching Guidance}
...
\end{equation}
where $\beta$ clips the weight for numerical stability
\cite{black2025realtime}. Reverse-mode automatic differentiation evaluates the VJP
through $f_\theta^\tau$; differentiability of $h$ and $h^\dagger$ is not required.
