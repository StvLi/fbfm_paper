...
- [ ] Complete Appendix D with matched final records for all selected 42 RoboTwin tasks under both clean and randomized configurations for LingBot-VA Base/FBFM; the consolidated ledger now completes all Base cells and 41/42 FBFM cells, while `handover_block` remains at 1/6 Clean and 0/4 Randomized. Keep the eight longer excluded tasks outside the main tables.
...
- [ ] Populate the LingBot-VA main Results table only after its detailed Appendix D records are complete; report only the final clean/randomized success rates and do not substitute non-matched pooled snapshot rates.
