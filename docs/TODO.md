...
- [ ] Freeze and report the exact revisions used for the final tables, together with upstream model versions, checkpoint artifact identifiers, dependency environments, accelerator hardware, and random seeds. The corrected DreamZero implementation is branch `experiment/dreamzero-l1mass-state-weight`, numerical commit `cb08c9e552730d26cc446885e79a3e270a270d0c`, with audit/documentation HEAD `0f2cc4f133532af16841b3698e7cc7a036cecdee`; it supersedes `a7dcd4a4bbf69709c038fb433bbc1cf42b029f63` for implementation claims. Do not reproduce this track from the default `main` branch. The finalized LingBot-VA/RoboTwin result packages identify evaluation revision `a116e48d9c8956c7ee66360ae68007bc146abcc3`; retain `3604b457a24485cddf326a997e48955b7ca6b548` only as the earlier theory-audit point. Hardware and dependency reporting still require consolidation.
...
- [x] Complete Appendix D for all selected 42 RoboTwin tasks under both clean and randomized configurations for LingBot-VA Base/FBFM. The final `handover_block` FBFM record is 1/10 Clean and 0/10 Randomized; the eight longer tasks remain outside the agreed matrix.
...
- [x] Resolve renderer aggregation scope: CPU and GPU rendering are treated as equivalent RoboTwin benchmark backends, source provenance remains explicit, and the 42-task headline uses equal-weight task-cell macro averages rather than mixed-count micro pooling or a seed-paired claim.
- [x] Replace the incomplete 41-task LingBot-VA aggregate with the full 42-task Clean/Randomized macro summary after completion of `handover_block`.
