#!/usr/bin/env python3
"""Atomically aggregate resumable LIBERO-40 worker shards."""
...
def manifest_task_keys(manifest: dict) -> list[str]:
    if "task_keys" in manifest:
        return list(manifest["task_keys"])
    return [
        f"{suite}:{task_id}"
        for suite in manifest["suites"]
        for task_id in manifest["task_ids_by_suite"][suite]
    ]
...
def main() -> None:
        ...
        if summary_path.exists():
            with summary_path.open(encoding="utf-8", newline="") as handle:
                for row in csv.DictReader(handle):
                    descriptions[f"{row['suite']}:{row['task_id']}"] = row[
                        "task_description"
                    ]
        for task_key in manifest_task_keys(manifest):
    ...
    if set(locations) != set(expected) or len(expected) != len(set(expected)):
        raise ValueError(
            f"shard task set mismatch: expected={expected}, "
            f"observed={sorted(locations)}"
        )
    ...
    for task_key in expected:
        ...
        if len(records) > target_trials:
            raise ValueError(f"too many trial records in {task_dir}: {len(records)}")
        ...
        task_rows.append(
            {
                ...
                "status": (
                    "complete"
                    if len(records) == target_trials
                    else ("running" if records else "pending")
                ),
                "trials": len(records),
        ...
        for record in records:
            if not record.get("actions_finite", False):
                raise ValueError(f"non-finite actions in {task_dir}: {record}")
            trial_rows.append(
                {
                    "suite": suite,
                    "task_id": task_id,
    ...
    summary = {
        ...
        "state_feedback_kp": reference["state_feedback_kp"],
        "effective_state_weight": reference["effective_state_weight"],
        "tasks": len(expected),
    ...
    rate = f"{100 * successes / episodes:.2f}%" if episodes else "-"
    lines = [
        ...
        "",
        f"State feedback kp: `{summary['state_feedback_kp']}`",
        "",
        "| Complete tasks | Episodes | Successes | Micro rate |",
        "| ---: | ---: | ---: | ---: |",
        f"| {complete_tasks}/{len(expected)} | {episodes}/{summary['expected_episodes']} | {successes} | {rate} |",
        "",
